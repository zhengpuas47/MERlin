...
class PixelBasedDecoder(object):
    ...
    def extract_barcodes_with_index(
        ...
        ## TODO: between is depricated, use greater and less instead.

        fullDF = fullDF[fullDF['x'].gt(cropWidth) &
                        fullDF['x'].lt(decodedImage.shape[0] - cropWidth) &
                        fullDF['y'].gt(cropWidth) &
                        fullDF['y'].lt(decodedImage.shape[0] - cropWidth) &                        
    ...
    def extract_refactors(
        ...
        for b in range(self._barcodeCount):
            barcodeRegions = [x for x in measure.regionprops(
                        measure.label((decodedImage == b).astype(np.int32)))
